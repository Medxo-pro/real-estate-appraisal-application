...
public record MockSystemOut(PrintStream mockOutput, BufferedReader terminal) {
  ...
  /**
   * Generate a new MockSystemOut object with freshly connected streams for the caller to provide
   * the callee. If the callee treats the resulting mockOutput as if it were System.out (or
   * System.err), it will not know the difference.
   *
   ...
   * @throws IOException if such an exception is generated when creating the streams
   */
  public static MockSystemOut build() throws IOException {
